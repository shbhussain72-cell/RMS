...
export default function QuickReplyRow({
  ...
  if (!options.length) {
    return (
      <p className="text-[13px] text-[#8a938e]" style={{ fontFamily: FONT_SANS }} {...tx('Nothing available right now.')} />
    )
  }
      ...
      <div className="flex flex-col gap-[8px]">
        {options.map((o) => (
          <button
            ...
            className="group flex items-center gap-[12px] rounded-[14px] border border-solid border-[#e7dfc9] bg-white px-[14px] py-[12px] text-start transition-all hover:border-[#1f5a44] hover:bg-[#f4faf6] hover:shadow-[0_4px_14px_-8px_rgba(21,64,47,0.35)] disabled:cursor-not-allowed disabled:opacity-40"
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[14px] font-bold text-[#23302a]" {...tx(o.label)} />
              {o.sublabel && (
        ...
        {options.map((o) => (
          ...
          >
            ...
            <span className="min-w-0">
              <span className="block text-[13.5px] font-bold leading-[17px] text-[#23302a]" {...tx(o.label)} />
    ...
    <div className="flex flex-wrap gap-[8px]">
      {options.map((o) => (
        <button
          ...
          disabled={o.disabled}
          onClick={() => onPick(o.value)}
          className="flex flex-col items-start rounded-[14px] border border-solid border-[#e7dfc9] bg-white px-[14px] py-[9px] text-start transition-colors hover:border-[#1f5a44] hover:bg-[#f4faf6] disabled:cursor-not-allowed disabled:opacity-40"
          style={{ fontFamily: FONT_SANS }}
        >
          <span className="text-[13.5px] font-bold text-[#23302a]" {...tx(o.label)} />
