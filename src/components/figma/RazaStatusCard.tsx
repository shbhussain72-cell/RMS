...
function RazaThumbnail() {
  ...
  return (
    <div className="h-[76px] w-[60px] shrink-0 overflow-hidden rounded-[10px] border border-[#e3cd96] bg-[#fdf8ee] shadow-[0_2px_8px_-4px_rgba(21,64,47,0.3)]">
      <img src="/figma/raza-letter.jpg" alt={t('Raza letter')} className="size-full object-cover" />
    </div>
  )
}
  ...
  if (!issued) {
    return (
      <div className={`flex h-[48px] items-center justify-between rounded-[12px] px-[16px] ${className}`} style={{ background: '#fff6e5' }}>
        <span className="text-[15px] text-[#3d3d3a]" style={{ fontFamily: FONT, fontWeight: 500 }} {...tx('Raza status')} />
        <span className="flex items-center gap-[7px]">
          <span className="size-[7px] shrink-0 rounded-full bg-[#b8821e]" />
          <span className="text-[15px] font-extrabold tracking-[0.2px] text-[#b8821e]" style={{ fontFamily: FONT }} {...tx('Pending')} />
        </span>
      </div>
    )
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-wrap items-baseline gap-x-[10px] gap-y-[2px]">
        <span className="text-[17px] text-[#23302a]" style={{ fontFamily: 'Marcellus, Georgia, serif' }} {...tx('Raza status')} />
        <span className="flex items-center gap-[6px]">
          <span className="size-[7px] shrink-0 rounded-full bg-[#1f7a4d]" />
          <span className="text-[17px] font-extrabold tracking-[0.2px] text-[#1f7a4d]" style={{ fontFamily: FONT }} {...tx('Issued')} />
        </span>
      </div>
      {onView && (
        <button
